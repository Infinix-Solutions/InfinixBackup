...
useHead({ title: `${t('sources.title')} — Infinix Backup` })

const { data: sources, refresh } = await useFetch<ApiSource[]>('/api/sources', { default: (): ApiSource[] => [] })
const search = ref('')
...
const filteredSources = computed(() => {
  if (!search.value.trim()) return sources.value
...
async function deleteSource(id: string) {
  ...
  toast.add({ title: t('sources.deleted'), color: 'success', icon: 'i-lucide-check-circle' })
  deleteId.value = null
  refresh()
}
...
async function testSource(id: string) {
  try {
    await $fetch(`/api/sources/${id}/test`, { method: 'POST' })
    ...
    <UCard class="anim-card">
      <template #header>
        <div class="flex items-center justify-between gap-3">
          <span class="text-sm text-muted">
            ...
            <span class="lowercase">{{ t('sources.title') }}</span>
          </span>
          <UInput
            v-model="search"
            size="sm"
      ...
      >
        <template #name-cell="{ row }">
          <div class="flex items-center gap-2.5">
            <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-elevated">
              <UIcon
                :name="SOURCE_TYPE_ICONS[row.original.type] || 'i-lucide-database'"
        ...
        <template #type-cell="{ row }">
          <UBadge
            :label="SOURCE_TYPE_LABELS[row.original.type] || row.original.type"
            color="neutral"
            variant="subtle"
            size="sm"
          />
          ...
          <div class="flex items-center gap-0.5 justify-end">
            <UTooltip :text="t('common.test_connection')">
              <UButton
                ...
                color="neutral"
                variant="ghost"
                @click="testSource(row.original.id)"
              />
            </UTooltip>
