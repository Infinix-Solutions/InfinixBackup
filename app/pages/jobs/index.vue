    ...
    <UCard class="anim-card">
      ...
      >
        ...
        <p class="text-xs text-muted mb-4">
          Utwórz pierwsze zadanie backupu aby zacząć.
        </p>
        <UButton
          to="/jobs/new"
          size="sm"
      ...
      >
        ...
        <UButton
          ...
          variant="ghost"
          color="neutral"
          @click="search = ''; enabledFilter = 'all'"
        >
          Wyczyść filtry
        </UButton>
      </div>
        ...
        <template #sourceName-cell="{ row }">
          <div class="flex items-center gap-1.5">
            <div class="flex h-5 w-5 shrink-0 items-center justify-center rounded bg-elevated">
              <UIcon
                :name="(row.original.sourceType && SOURCE_TYPE_ICONS[row.original.sourceType]) || 'i-lucide-database'"
          ...
          <div class="flex items-center gap-0.5 justify-end">
            <UTooltip :text="t('common.run_now')">
              <UButton
                ...
                color="primary"
                variant="ghost"
                :loading="runningJobs.has(row.original.id)"
                @click="runNow(row.original.id, row.original.name)"
              />
            </UTooltip>
            <UTooltip :text="row.original.enabled ? t('common.disabled') : t('common.enabled')">
              <UButton
                :icon="row.original.enabled ? 'i-lucide-pause' : 'i-lucide-play-circle'"
                size="xs"
                color="neutral"
                variant="ghost"
