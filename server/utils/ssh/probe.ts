...
(docker ps --format '{{.Names}}' 2>/dev/null | while read n; do echo "container=$n"; done || true);
...
export async function probeServer(config: SshConfig): Promise<SshProbeResult> {
  ...
  const result: SshProbeResult = { docker: false, postgres: false, mysql: false, mongo: false, containers: [] }
